...
$.extend(SOY2_DatePicker.prototype,{
	...
	buildElement : function(){
		...
		var max_year = (this.show_future) ? this.today.getFullYear() + 20 : this.today.getFullYear();
		for(var i=(current_year-20);i<=max_year;i++){
		...
		$(".today_btn",ele).bind("click",function(){
			...
			inst.selectValue(inst.today);
			
			inst.check(
				inst.today.getFullYear() + "-" + 
				(inst.today.getMonth() + 1) + "-" + 
				inst.today.getDate()
			);
		});
...
$.extend(SOY2_TimeSelect,{
	 show : function(ele){
		
		ele = $(ele);
		
		if(ele.next(".soy2_time_select").length < 1){
			this.buildSelect(ele);
		}
		
		wrap = ele.next(".soy2_time_select"); 
		wrap.find("select").val(ele.val());
	...
	,buildSelect : function(ele){
		var div = $("<div class='soy2_time_select'></select>");
		...
		select.bind("change",function(){
			$(ele).val($(this).val());
			SOY2_TimeSelect.hide(null);
		});
	}
	
	,hide : function(ele){
		if (ele) {
			if ($(ele).hasClass("time-input")) 				
				return;
			if ($(ele).parents().hasClass("soy2_time_select")) 				
				return;
		}
		$(".soy2_time_select").hide();
	} 
});

$(function(){

	$("input.date-input").bind("focus",function(e){
		SOY2_DatePicker.show(this);
	});
	
	$("input.time-input").bind("focus",function(e){
		SOY2_TimeSelect.show(this);
	});

	$("body").bind("click",function(e){
		SOY2_TimeSelect.hide($(e.target));
		if($(e.target).hasClass("date-input"))return;
		if($(e.target).parents().hasClass("soy2_date_picker"))return;
		SOY2_DatePicker.hideAll();
	});

	$("input.date-input").bind("keyup",function(e){
